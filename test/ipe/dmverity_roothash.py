...
_policy_name = "dmverity_roothash"

class DMVerityRootHashTests(IPETestCase):
    ...
    def setUpClass(cls):
        """
            ...
            Performs the following:
                1. Map the test.squashfs volume to /dev/mapper/{cls._dev}
                2. Mount the mapped volume to cls.__mnt
        ...
        util._exec("veritysetup", argv=["open",
                                    ...
                                    f"{cls._dev}",
                                    f"{cls._bin}.hashtree",
                                    root_hash], check=True)
        makedirs(cls._mnt, exist_ok=True)
        info(f"Mounting /dev/mapper/{cls._dev}")
        util._exec("mount", argv=[f"/dev/mapper/{cls._dev}", cls._mnt], check=True)
        cls._allow = cls._mnt
        ...
        util.new_ipe_poilcy(cls._securityfs, f"{cls._policy}/test_func/p7s/dmverity_roothash.p7s")
    ...
    def tearDownClass(cls):
        """
            ...
            Performs the following:
                ...
                2. Unmount the mapped volume to cls.__mnt
