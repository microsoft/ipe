...
static struct ipe_parsed_policy *new_parsed_policy(void)
{
	...
	p->global_default_action = __IPE_ACTION_MAX;

	for (i = 0; i < ARRAY_SIZE(p->rules); ++i) {
		t = &p->rules[i];

		t->default_action = __IPE_ACTION_MAX;
...
static int parse_rule(char *line, struct ipe_parsed_policy *p)
{
	...
	enum ipe_op_type op = __IPE_OP_MAX;
	enum ipe_action_type action = __IPE_ACTION_MAX;
	...
	if (is_default_rule) {
		if (op == __IPE_OP_MAX) {
			if (p->global_default_action != __IPE_ACTION_MAX)
				rc = -EBADMSG;
			else
				p->global_default_action = action;
		} else {
			if (p->rules[op].default_action != __IPE_ACTION_MAX)
				rc = -EBADMSG;
			else
				p->rules[op].default_action = action;
		}
	} else if (op != __IPE_OP_MAX && action != __IPE_ACTION_MAX) {
		r->op = op;
		r->action = action;
	} else {
		rc = -EBADMSG;
...
static int validate_policy(const struct ipe_parsed_policy *p)
{
	...
	for (i = 0; i < ARRAY_SIZE(p->rules); ++i) {
		if (p->rules[i].default_action == __IPE_ACTION_MAX)
			return -EBADMSG;
	}
