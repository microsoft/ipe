...
- Integrity Policy Enforcement (IPE).  IPE supports enforcing access
  ...
  including fs-verity enabled files. "IPE policy" allows for specific
  authorization of fs-verity files, utilizing properties such as
  `fsverity_digest` for identifying files by their verity digest, and
  `fsverity_signature` to ensure files are signed. This makes IPE
